...
class _PantallaReservarHorarioState extends State<PantallaReservarHorario> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Reservar')), backgroundColor: Colors.white,
      body: SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.all(16.0),
        child: Container(width: double.infinity, padding: const EdgeInsets.all(24),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start,
            ...
            children: [
              Text(widget.area.toUpperCase(), style: GoogleFonts.inter(fontSize: 22, fontWeight: FontWeight.bold,),),
              ...
              Text('Hora', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.bold,),),
              const SizedBox(height: 8),

              //ELEGIR HORA (solo horas en punto, sin minutos)
              TextField(controller: _horaController, readOnly: true, decoration: InputDecoration(border: const OutlineInputBorder(), hintText: 'HH:00', suffixIcon: const Icon(Icons.access_time), errorText: _horaError,),
                onTap: () async {final TimeOfDay? horaElegida = await showTimePicker(context: context,
                  initialTime: TimeOfDay.now(),
                );
                if (horaElegida != null) {
                  final String horaFormateada = '${horaElegida.hour.toString().padLeft(2, '0')}:00';
              ...
              Text('Solo se permiten horas en punto. Cada reserva dura 2 horas.', style: GoogleFonts.inter(fontSize: 12, color: Colors.black54,),),
              const SizedBox(height: 32),
              ...
              Center(
                child: SizedBox(
                  ...
                  onPressed: () {
                    ...
                    //RESTRICCIONES DE CAMPOS VACIOS
                    setState(() {
                    ...
                    //VALIDACION DE HORARIO OCUPADO
                    final bool ocupado = ReservasStore.horarioOcupado(
                      area: widget.area,
                    ...
                    if (ocupado) {
                      setState(() {
                        _horaError = 'Este horario ya está reservado para esta área';
                      });
                      return;
                    }
                    ...
                    Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => PantallaConfirmacionReserva(area: widget.area, fecha: _fechaController.text, hora: _horaController.text,),),);
                  ...
                  child: Text('Reservar', style: GoogleFonts.inter(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold,),),
                ),
                ),
              ),
