...
class PantallaMisReservas extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Mis Reservas')), backgroundColor: Colors.white,
      body: SafeArea(child: Padding(padding: const EdgeInsets.all(16.0),
          child: Container(width: double.infinity, padding: const EdgeInsets.all(16),
            ...
            child: ReservasStore.reservas.isEmpty
                ? Center(child: Text('Aún no tienes reservas', style: GoogleFonts.inter(fontSize: 14, color: Colors.black54,),),)
                : ListView.separated(itemCount: ReservasStore.reservas.length, separatorBuilder: (context, index) => const Divider(color: Colors.black, thickness: 1,),
              ...
              itemBuilder: (context, index) {
                ...
                return Padding(padding: const EdgeInsets.symmetric(vertical: 16),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                    ...
                    children: [
                      Text(reserva['area']!.toUpperCase(), style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.bold,),),
                      const SizedBox(height: 8),

                      Text('${reserva['fecha']} - ${reserva['hora']}', style: GoogleFonts.inter(fontSize: 13, color: Colors.black54,),),
                    ],
                  ),
                );
              },
